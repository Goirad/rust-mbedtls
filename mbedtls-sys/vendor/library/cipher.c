                         ...
                         unsigned char *tag, size_t tag_len )
{
    ...
    if( MBEDTLS_MODE_KW == ctx->cipher_info->mode || MBEDTLS_MODE_KWP == ctx->cipher_info->mode )
    {
        ...
        // This assumes the called sets the value of olen (out param) as the size of output buffer
        // TODO : Revisit this
        ...
        return( mbedtls_nist_kw_wrap( ctx->cipher_ctx, mode, input, ilen, output, olen, out_size ) );
    }
                         ...
                         const unsigned char *tag, size_t tag_len )
{
    ...
    if( MBEDTLS_MODE_KW == ctx->cipher_info->mode || MBEDTLS_MODE_KWP == ctx->cipher_info->mode )
    {
        ...
        return( mbedtls_nist_kw_unwrap( ctx->cipher_ctx, mode, input, ilen, output, olen, out_size ) );
    }
#endif /* MBEDTLS_NIST_KW_C */
